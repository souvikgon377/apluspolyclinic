...
const Prescriptions = () => {
  ...
  const handleAppointmentClick = (item) => {
    ...
    // Open modal on mobile screens (less than 1024px)
  ...
  return (
    <div className='w-full max-w-6xl mx-auto px-4 py-8 overflow-x-hidden'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        ...
        {/* Upload/View Section - Desktop (hidden on mobile) */}
      ...
      {showModal && selectedAppointment && (
        <div className='lg:hidden fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4'>
          <div className='bg-white w-full max-h-[90vh] sm:max-h-[85vh] sm:rounded-2xl rounded-t-2xl overflow-hidden flex flex-col animate-slide-up'>
            {/* Modal Header */}
            <div className='flex items-center justify-between p-4 border-b border-gray-200 bg-white sticky top-0 z-10'>
              ...
              <div className='flex items-center gap-2'>
                {selectedAppointment?.prescription && (
                  <button
                    onClick={() => setViewMode(!viewMode)}
                    className='px-3 py-1.5 text-xs bg-emerald-100 text-primary rounded-lg hover:bg-emerald-200 transition-colors font-medium'
                  >
                    {viewMode ? 'Upload New' : 'View'}
                  </button>
                )}
                <button
                  onClick={closeModal}
                    ...
                    <div>
                      ...
                      <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary transition-colors'>
                        <input
                          ...
                          onChange={handleImageChange}
                          className='hidden'
                          id='prescription-upload-mobile'
                        />
                        <label htmlFor='prescription-upload-mobile' className='cursor-pointer'>
                          {prescriptionImage ? (
                            <div className='space-y-2'>
                              <img 
                                src={URL.createObjectURL(prescriptionImage)} 

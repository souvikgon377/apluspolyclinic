...
const userModel = {
  ...
  async create(userData) {
    ...
    return { _id: docRef.id, ...userWithDefaults };
  },
  ...
  async findOne(query) {
    ...
    const snapshot = await usersCollection.where(field, '==', value).limit(1).get();
  ...
  async deleteMany(query = {}) {
    ...
    users.forEach(user => {
      batch.delete(usersCollection.doc(user._id));
    });
    await batch.commit();
